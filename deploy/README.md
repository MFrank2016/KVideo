...
1. Run `scripts/prepare-env.sh` to copy examples into the ignored runtime files (`.env`, `kvideo/.env.build`, `kvideo/ad_keywords.txt`, `danmu_api/config/.env`) during the first setup. The script skips existing files so you can re-run it safely after editing templates only when you delete the generated artifacts first.
